...
async function getGrantedFeatures(trx: Db, tenantId: string): Promise<Set<string>> {
...
export async function apiKeysRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/', async (request) => {
    ...
    return withTenant(user.tenant_id, async (trx) => {
      const keys = await trx.selectFrom('api_keys')
  ...
  fastify.post<{
    ...
    return withTenant(user.tenant_id, async (trx) => {
      const granted = await getGrantedFeatures(trx, user.tenant_id);
      ...
      const row = await trx.insertInto('api_keys')
        .values({
  ...
  fastify.delete<{ Params: { id: string } }>('/:id', async (request, reply) => {
    ...
    return withTenant(user.tenant_id, async (trx) => {
      const key = await trx.selectFrom('api_keys').select('id').where('id', '=', id).where('tenant_id', '=', user.tenant_id).executeTakeFirst();
      if (!key) return reply.status(404).send({ error: 'API key not found' });

      await trx.updateTable('api_keys').set({ revoked_at: new Date() }).where('id', '=', id).where('tenant_id', '=', user.tenant_id).execute();
      reply.status(204);
      return null;
    });
  });
  ...
  fastify.get('/usage', async (request) => {
    ...
    return withTenant(user.tenant_id, async (trx) => {
      const rows = await trx.selectFrom('api_usage_events')
