      ...
      <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid var(--border)', marginBottom: 20, flexWrap: 'wrap' }}>
        {TABS.map(t => (
          <button key={t} type="button" onClick={() => setTab(t)}
            style={{ padding: 'var(--ds-btn-py) 16px', border: 'none', borderBottom: tab === t ? '2px solid var(--teal)' : '2px solid transparent', background: 'none', color: tab === t ? 'var(--ink)' : 'var(--ink3)', fontWeight: tab === t ? 700 : 500, fontSize: 13, cursor: 'pointer', minHeight: 'var(--ctl-h)', boxSizing: 'border-box'}}>
            {t}
          </button>
        ))}
      </div>
              ...
              {!editing ? (
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button type="button" onClick={() => setEditing(false)}
                    style={{ fontSize: 12, fontWeight: 600, color: 'var(--ink3)', background: 'none', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: 'var(--ds-btn-py-xs) 12px', cursor: 'pointer', minHeight: 'var(--ctl-h-xs)', boxSizing: 'border-box'}}>Cancel</button>
                  <button type="button" onClick={saveEdit} disabled={saving}
                    style={{ fontSize: 12, fontWeight: 600, color: '#fff', background: 'var(--teal)', border: 'none', borderRadius: 'var(--r)', padding: 'var(--ds-btn-py-xs) 12px', cursor: 'pointer', opacity: saving ? 0.6 : 1, minHeight: 'var(--ctl-h-xs)', boxSizing: 'border-box'}}>
                    {saving ? 'Saving…' : 'Save'}
                  </button>
                </div>
              )}
    ...
    return (
      <div style={{ ...cardStyle, padding: '40px 20px', textAlign: 'center' }}>
        ...
        <button type="button" onClick={createDefaultPlan} disabled={creating}
          style={{ padding: 'var(--ds-btn-py) 18px', borderRadius: 'var(--r)', border: 'none', background: 'var(--teal)', color: '#fff', fontWeight: 600, fontSize: 13, cursor: 'pointer', opacity: creating ? 0.6 : 1, minHeight: 'var(--ctl-h)', boxSizing: 'border-box'}}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={cardStyle}>
          ...
          <form onSubmit={addItem} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <button type="submit" style={{ padding: 'var(--ds-btn-py) 12px', borderRadius: 'var(--r)', border: 'none', background: 'var(--teal)', color: '#fff', fontWeight: 600, fontSize: 12, cursor: 'pointer', minHeight: 'var(--ctl-h)', boxSizing: 'border-box'}}>Add item</button>
          </form>
        </div>
        ...
        <div style={cardStyle}>
          ...
          <button type="button" onClick={pack} disabled={packing || items.length === 0}
            style={{ marginTop: 10, width: '100%', padding: 'var(--ds-btn-py) 12px', borderRadius: 'var(--r)', border: 'none', background: 'var(--ink)', color: '#fff', fontWeight: 700, fontSize: 12, cursor: items.length === 0 ? 'default' : 'pointer', opacity: items.length === 0 ? 0.5 : 1, minHeight: 'var(--ctl-h)', boxSizing: 'border-box'}}>
