...
export const Chat: React.FC = () => {
  ...
  function SecHdr({ label, sk, count }: { label: string; sk: keyof typeof openSecs; count: number }) {
    return (
      <button type="button" onClick={() => setOpenSecs(p => ({ ...p, [sk]: !p[sk] }))} style={{ display: 'flex', alignItems: 'center', gap: 4, width: '100%', background: 'none', border: 'none', cursor: 'pointer', padding: '10px 10px 3px', color: 'var(--ink3)', fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
    ...
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden', background: 'var(--white)', fontFamily: 'var(--font)' }}>
      ...
      <div style={{ width: 244, flexShrink: 0, display: 'flex', flexDirection: 'column', borderRight: '1px solid var(--border)', background: 'var(--white)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '12px 14px 10px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 9 }}>
          ...
          <button type="button" onClick={() => setCreating('channel')} title="New channel" style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 5, borderRadius: 5, color: 'var(--ink3)', display: 'flex' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 4px 8px' }}>
          ...
          <button type="button" onClick={() => setCreating('channel')} style={{ display: 'flex', alignItems: 'center', gap: 7, width: '100%', padding: '4px 10px', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ink3)', fontSize: 12, fontFamily: 'var(--font)', borderRadius: 6 }}>
          ...
          <button type="button" onClick={() => setCreating('dm')} style={{ display: 'flex', alignItems: 'center', gap: 7, width: '100%', padding: '4px 10px', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ink3)', fontSize: 12, fontFamily: 'var(--font)', borderRadius: 6 }}>
          ...
          <button type="button" onClick={() => setCreating('group')} style={{ display: 'flex', alignItems: 'center', gap: 7, width: '100%', padding: '4px 10px', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ink3)', fontSize: 12, fontFamily: 'var(--font)', borderRadius: 6 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        {msg.reactions.length > 0 && (
                          <div style={{ display: 'flex', gap: 4, marginTop: 5, flexWrap: 'wrap' }}>
                            {msg.reactions.map(r => (
                              <button type="button" key={r.emoji} onClick={() => react(msg.id, r.emoji)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '2px 8px', borderRadius: 9, border: `1px solid ${r.mine ? 'var(--teal-m)' : 'var(--border)'}`, background: r.mine ? 'var(--teal-l)' : 'var(--bg)', cursor: 'pointer', fontSize: 13, fontFamily: 'var(--font)', transition: 'all 0.1s' }}>
                                {r.emoji}<span style={{ fontSize: 11, fontWeight: 600, color: r.mine ? 'var(--teal)' : 'var(--ink2)' }}>{r.count}</span>
                              </button>
                            ))}
                            <button type="button" onClick={() => react(msg.id, '👍')} style={{ padding: '2px 8px', borderRadius: 9, border: '1px solid var(--border)', background: 'var(--bg)', cursor: 'pointer', fontSize: 12, color: 'var(--ink3)', fontFamily: 'var(--font)' }}>+</button>
                          </div>
                        )}
                      </div>
                    </div>
              ...
              <div style={{ border: '1.5px solid var(--border)', borderRadius: 9, overflow: 'visible', background: 'var(--white)', transition: 'border-color 0.15s' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', padding: '5px 8px', gap: 1, borderTop: '1px solid var(--border)' }}>
                  <div ref={emojiRef} style={{ position: 'relative' }}>
                    <button type="button" onClick={() => setShowEmoji(p => !p)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '5px 6px', borderRadius: 6, color: showEmoji ? 'var(--teal)' : 'var(--ink3)', display: 'flex', alignItems: 'center' }}>
                      ...
                      <div style={{ position: 'absolute', bottom: 'calc(100% + 8px)', left: 0, background: 'var(--white)', border: '1px solid var(--border)', borderRadius: 9, padding: 8, display: 'grid', gridTemplateColumns: 'repeat(8,1fr)', gap: 2, boxShadow: '0 4px 20px rgba(0,0,0,0.12)', zIndex: 100 }}>
                        {EMOJIS.map(em => (
                          <button type="button" key={em} onClick={() => { setInput(p => p + em); setShowEmoji(false); inputRef.current?.focus(); }}
                            style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 18, padding: '5px', borderRadius: 6, lineHeight: 1 }}>
                            {em}
                          </button>
                        ))}
                      </div>
                  ...
                  <button type="button" onClick={send} disabled={!input.trim() || sending} style={{ background: input.trim() ? 'var(--teal)' : 'var(--border)', border: 'none', borderRadius: 7, padding: '6px 14px', cursor: input.trim() ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', gap: 6, color: input.trim() ? '#fff' : 'var(--ink3)', fontSize: 13, fontWeight: 600, fontFamily: 'var(--font)' }}>
        ...
        <>
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, height: '100vh', width: 380, maxWidth: '92vw', zIndex: 1401, background: 'var(--white)', borderLeft: '1px solid var(--border)', boxShadow: '-8px 0 32px rgba(15,23,42,0.12)', display: 'flex', flexDirection: 'column', padding: 24, overflowY: 'auto' }}>
                  ...
                  {staff.map(s => {
                    ...
                    return (
                      <button type="button" key={s.id} onClick={() => setNewMemberIds(p => on ? p.filter(id => id !== s.id) : [...p, s.id])}
                        style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '9px 12px', border: 'none', borderBottom: '1px solid var(--bg)', background: on ? 'var(--teal-l)' : 'transparent', cursor: 'pointer', textAlign: 'left' }}>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button type="button" onClick={() => { setCreating(null); setNewName(''); setNewMemberIds([]); }} style={{ padding: '8px 16px', borderRadius: 9, border: '1px solid var(--border)', background: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600, color: 'var(--ink2)', fontFamily: 'var(--font)' }}>Cancel</button>
              <button type="button" onClick={createChannelOrGroup} disabled={!newName.trim() || (creating === 'group' && newMemberIds.length === 0)}
                style={{ padding: '8px 18px', borderRadius: 9, border: 'none', background: newName.trim() ? 'var(--teal)' : 'var(--border)', color: newName.trim() ? '#fff' : 'var(--ink3)', cursor: newName.trim() ? 'pointer' : 'not-allowed', fontSize: 13, fontWeight: 600, fontFamily: 'var(--font)' }}>
            ...
            <div style={{ border: '1px solid var(--border)', borderRadius: 9, overflow: 'hidden' }}>
              ...
              {staff.map(s => (
                <button type="button" key={s.id} onClick={() => startDm(s.id)}
                  style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '10px 12px', border: 'none', borderBottom: '1px solid var(--bg)', background: 'transparent', cursor: 'pointer', textAlign: 'left' }}>
