    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={{ ...cardStyle, marginBottom: 20, display: 'flex', alignItems: 'flex-end', gap: 12, flexWrap: 'wrap' }}>
        ...
        <button type="button" onClick={loadPreview} disabled={loadingPreview}
          style={{ padding: '9px 16px', borderRadius: 9, border: '1px solid var(--border)', background: 'var(--white)', color: 'var(--ink)', fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>
          {loadingPreview ? 'Loading…' : 'Preview'}
        </button>
        <button type="button" onClick={() => preview && generateReportPDF(reportType, from, to, preview, vehicleName)} disabled={!preview}
          style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 16px', borderRadius: 9, border: 'none', background: 'var(--teal)', color: '#fff', fontWeight: 700, fontSize: 13, cursor: preview ? 'pointer' : 'default', opacity: preview ? 1 : 0.5 }}>
