          ...
          <div style={{ padding: '8px' }}>
            {TABS.map(t => (
              <button key={t.key} type="button" onClick={() => setTab(t.key)}
                style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: 'var(--ds-btn-py) 12px', background: tab === t.key ? 'var(--bg)' : 'transparent', border: 'none', borderRadius: 'var(--r)', cursor: 'pointer', color: tab === t.key ? 'var(--teal)' : 'var(--ink2)', fontWeight: tab === t.key ? 600 : 400, fontFamily: 'var(--font)', fontSize: 12.5, marginBottom: 1 }}>
