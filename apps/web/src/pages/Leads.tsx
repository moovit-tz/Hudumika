            ...
            {PROF_TABS.map(tab => {
              ...
              return (
                <button key={tab.key} type="button" onClick={() => { setProfileTab(tab.key); setEditMode(false); }}
                  style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '12px 16px', border: 'none', borderBottom: isActive ? '2px solid var(--teal)' : '2px solid transparent', background: 'none', color: isActive ? 'var(--teal)' : 'var(--ink3)', fontSize: 13, fontWeight: isActive ? 700 : 500, cursor: 'pointer', fontFamily: 'var(--font)', whiteSpace: 'nowrap', marginBottom: -1 }}
                  ...
                  <div style={{ display: 'flex', gap: 8, marginTop: 14, flexWrap: 'wrap' }}>
                    {STAGES.filter(s => s !== sel.stage).map(s => (
                      <button key={s} type="button"
                        ...
                        style={{ padding: '5px 12px', border: '1px solid var(--border)', borderRadius: 7, background: 'var(--bg)', cursor: 'pointer', fontSize: 11.5, fontWeight: 600, fontFamily: 'var(--font)', color: 'var(--ink2)' }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderTop: '1px solid var(--border)' }}>
              ...
              <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                <button type="button" aria-label="Previous page" onClick={() => setPage(p => Math.max(1, p - 1))} disabled={pg === 1}
                  style={{ border: '1px solid var(--border)', borderRadius: 6, padding: '4px 8px', background: 'var(--white)', cursor: pg === 1 ? 'default' : 'pointer', color: 'var(--ink3)', opacity: pg === 1 ? 0.4 : 1 }}>
                ...
                {getPageNums(pg, totalPages).map((p, i) =>
                  ...
                  <button key={p} type="button" onClick={() => setPage(Number(p))}
                    style={{ border: `1px solid ${pg === p ? 'var(--teal)' : 'var(--border)'}`, borderRadius: 6, padding: '4px 10px', background: pg === p ? 'var(--teal)' : 'var(--white)', cursor: 'pointer', fontSize: 13, color: pg === p ? '#fff' : 'var(--ink)', minWidth: 32 }}>
                    {p}
                  </button>
                )}
                <button type="button" aria-label="Next page" onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={pg === totalPages}
                  style={{ border: '1px solid var(--border)', borderRadius: 6, padding: '4px 8px', background: 'var(--white)', cursor: pg === totalPages ? 'default' : 'pointer', color: 'var(--ink3)', opacity: pg === totalPages ? 0.4 : 1 }}>
