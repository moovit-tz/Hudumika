...
const SEVERITY_CFG: Record<string, { color: string; bg: string }> = {
  ...
  HIGH: { color: 'var(--teal)', bg: 'rgba(234,88,12,0.14)' }, CRITICAL: { color: 'var(--red)', bg: 'rgba(220,38,38,0.14)' },
};
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
        {STATUS_FILTERS.map(s => (
          <button key={s} type="button" onClick={() => setStatusFilter(s)}
            style={{ padding: '5px 12px', borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: `1px solid ${statusFilter === s ? 'var(--teal)' : 'var(--border)'}`, background: statusFilter === s ? 'var(--teal-l)' : 'var(--white)', color: statusFilter === s ? 'var(--teal)' : 'var(--ink3)' }}>
