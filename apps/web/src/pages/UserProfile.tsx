...
function Toggle({ on, onChange }: { on: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      type="button"
      onClick={() => onChange(!on)}
      style={{ width: 42, height: 24, borderRadius: 9, background: on ? 'var(--teal)' : 'var(--border)', border: 'none', cursor: 'pointer', position: 'relative', transition: 'background 0.2s', flexShrink: 0 }}
    >
    ...
    <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', background: 'var(--bg)', fontFamily: 'var(--font)', padding: isMobile ? '16px' : '32px' }}>
      ...
      <div className="profile-container">
        ...
        <div style={{ display: 'flex', padding: '0 28px', borderTop: '1px solid var(--border)', gap: 0 }}>
          {TABS.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '11px 16px', border: 'none', background: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: 'var(--font)', color: activeTab === t.key ? 'var(--teal)' : 'var(--ink3)', borderBottom: activeTab === t.key ? '2px solid var(--teal)' : '2px solid transparent', marginBottom: -1 }}>
          ...
          <>
            <Card title="Change Password" subtitle="Use a strong password with letters, numbers and symbols.">
              <form onSubmit={handlePwSave}>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
                  <button type="button" onClick={() => setPwForm({current:'',next:'',confirm:''})} style={{ padding: '9px 20px', border: '1.5px solid var(--border)', borderRadius: 9, background: 'var(--white)', cursor: 'pointer', fontSize: 13.5, fontWeight: 600, fontFamily: 'var(--font)', color: 'var(--ink)' }}>Cancel</button>
                  <button type="submit" disabled={pwSaving} style={{ padding: '9px 22px', border: 'none', borderRadius: 9, background: 'var(--teal)', color: '#fff', cursor: 'pointer', fontSize: 13.5, fontWeight: 700, fontFamily: 'var(--font)' }}>
            ...
            <Card title="Active Sessions" subtitle="Devices currently logged into your account.">
              {[
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 0', borderBottom: i === 0 ? '1px solid var(--border)' : 'none' }}>
                  ...
                  {!s.current && <button style={{ padding: '5px 12px', border: '1.5px solid var(--border)', borderRadius: 7, background: 'var(--white)', cursor: 'pointer', fontSize: 12.5, color: 'var(--red)', fontWeight: 600, fontFamily: 'var(--font)' }}>Revoke</button>}
                </div>
              ))}
            </Card>
            ...
            <Card title="Danger Zone" subtitle="Irreversible actions. Proceed with caution.">
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0' }}>
                ...
                <button style={{ padding: '7px 16px', border: '1.5px solid var(--border)', borderRadius: 9, background: 'var(--white)', cursor: 'pointer', fontSize: 13, fontWeight: 600, color: 'var(--gold)', fontFamily: 'var(--font)' }}>Deactivate</button>
              </div>
            </Card>
          </>
        )}
        ...
        {activeTab === 'notifications' && (
          <>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button type="button" style={{ padding: '9px 22px', border: 'none', borderRadius: 9, background: 'var(--teal)', color: '#fff', cursor: 'pointer', fontSize: 13.5, fontWeight: 700, fontFamily: 'var(--font)' }}>Save Preferences</button>
            </div>
          </>
        )}
        ...
        {activeTab === 'activity' && (
          <Card title="Login & Activity Log" subtitle="Recent account activity. Contact support if you notice anything suspicious.">
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 12 }}>
              <button style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', border: '1.5px solid var(--border)', borderRadius: 9, background: 'var(--white)', cursor: 'pointer', fontSize: 12.5, fontWeight: 600, fontFamily: 'var(--font)', color: 'var(--ink)' }}>
