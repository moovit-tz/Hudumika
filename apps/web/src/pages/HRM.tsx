...
/* -- Payslip Modal & PDF Generator -- */
type PayslipRow = { emp: string; basic: number; allow: number; ded: number; status: string };

function printPayslip(row: PayslipRow, monthLabel: string) {
  ...
  const empInitials = row.emp.split(' ').map(w => w[0]).join('').toUpperCase();
...
<html>
<head>
  <title>Payslip - ${row.emp} - ${monthLabel}</title>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { background: #f8fafc; color: #0f172a; font-family: 'Inter', system-ui, -apple-system, sans-serif; font-size: 13px; line-height: 1.5; padding: 32px 16px; }
    .toolbar { position: fixed; top: 16px; right: 16px; z-index: 100; display: flex; gap: 8px; }
    .toolbar button { font-family: inherit; font-size: 13px; font-weight: 700; border: 1px solid #cbd5e1; background: #fff; color: #1e293b; padding: 8px 16px; border-radius: 8px; cursor: pointer; box-shadow: 0 2px 6px rgba(0,0,0,0.08); transition: all 0.15s; }
    .toolbar button.primary { background: #0891b2; color: #fff; border-color: #0891b2; }
    .toolbar button.primary:hover { background: #0e7490; }
    .sheet { max-width: 720px; margin: 0 auto; background: #ffffff; border-radius: 12px; border: 1px solid #e2e8f0; box-shadow: 0 10px 30px rgba(0,0,0,0.08); padding: 40px; }
    .hdr { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 28px; padding-bottom: 20px; border-bottom: 2px solid #0891b2; }
    .company-title { font-family: 'Space Grotesk', sans-serif; font-size: 24px; font-weight: 800; color: #0f172a; letter-spacing: -0.02em; }
    .company-sub { font-size: 12px; color: #64748b; margin-top: 3px; }
    .slip-badge { font-family: 'Space Grotesk', sans-serif; font-size: 20px; font-weight: 800; color: #0891b2; letter-spacing: 0.05em; text-align: right; }
    .slip-period { font-size: 13px; color: #64748b; margin-top: 2px; text-align: right; }
    ...
    .sec-hdr { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px; padding: 6px 12px; border-radius: 4px; }
    .sec-hdr.earn { color: #0891b2; background: rgba(8,145,178,0.08); }
    .sec-hdr.ded { color: #ef4444; background: rgba(239,68,68,0.08); }
    table { width: 100%; border-collapse: collapse; }
    td { padding: 8px 4px; font-size: 13px; color: #334155; }
    td.num { font-weight: 600; text-align: right; font-variant-numeric: tabular-nums; }
    tr.border-bottom { border-bottom: 1px solid #f1f5f9; }
    tr.total td { padding: 10px 4px; font-weight: 700; color: #0f172a; border-top: 2px solid #e2e8f0; }
    tr.total td.num { font-size: 14px; font-weight: 800; }
    .net-hero { background: linear-gradient(135deg, #0f172a 0%, #1e3a5f 100%); border-radius: 10px; padding: 20px 24px; display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; color: #fff; }
    .net-title { font-size: 11px; font-weight: 700; color: rgba(255,255,255,0.6); text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px; }
    .net-sub { font-size: 12px; color: rgba(255,255,255,0.7); }
    .net-val { font-size: 26px; font-weight: 900; color: #ffffff; letter-spacing: -0.02em; font-variant-numeric: tabular-nums; }
    .tax-box { padding: 10px 14px; background: #fefce8; border: 1px solid #fef08a; border-radius: 6px; margin-bottom: 32px; font-size: 11px; color: #713f12; line-height: 1.5; }
    .signatures { display: grid; grid-template-columns: 1fr 1fr; gap: 40px; margin-top: 24px; padding-top: 20px; border-top: 1px solid #e2e8f0; }
    .sig-col { text-align: center; }
    .sig-line { height: 40px; border-bottom: 1px solid #94a3b8; margin-bottom: 6px; }
    .sig-title { font-size: 11px; color: #64748b; }
    .sig-name { font-size: 12px; font-weight: 600; color: #0f172a; margin-top: 2px; }
    .ftr { text-align: center; margin-top: 24px; font-size: 10px; color: #94a3b8; }
    @media print {
      body { background: #fff; padding: 0; }
      .toolbar { display: none !important; }
      .sheet { border: none; box-shadow: none; padding: 0; margin: 0; max-width: 100%; }
    }
  </style>
</head>
<body>
  ...
  <div class="sheet">
    <div class="hdr">
      <div>
        <div class="company-title">Moovit Logistics Ltd</div>
        <div class="company-sub">Plot 15 Haile Selassie Road, Dar es Salaam</div>
        <div class="company-sub">TIN: 100-200-300</div>
      </div>
      <div>
        <div class="slip-badge">PAYSLIP</div>
        <div class="slip-period">${monthLabel}</div>
      </div>
    </div>
    ...
    <div class="info-grid">
      <div>
        ...
        <div class="info-val">${row.emp}</div>
      </div>
      <div>
        <div class="info-label">Employee ID</div>
        <div class="info-val">EMP-${empInitials}-001</div>
      </div>
      <div>
        <div class="info-label">Pay Period</div>
        <div class="info-val">${monthLabel}</div>
      </div>
      <div>
        <div class="info-label">Payment Mode</div>
        <div class="info-val">Bank Transfer</div>
      </div>
    </div>

    <div class="grid-2">
      <div>
        <div class="sec-hdr earn">Earnings</div>
        <table>
          <tbody>
            <tr class="border-bottom">
              <td>Basic Salary</td>
              <td class="num">TZS ${row.basic.toLocaleString()}</td>
            </tr>
            <tr class="border-bottom">
              <td>Allowances</td>
              <td class="num">TZS ${row.allow.toLocaleString()}</td>
            </tr>
            <tr class="total">
              <td>Gross Pay</td>
              <td class="num" style="color:#0891b2;">TZS ${gross.toLocaleString()}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div>
        <div class="sec-hdr ded">Deductions</div>
        <table>
          <tbody>
            <tr class="border-bottom">
              <td>PAYE (Tax) ${paye === 0 ? '<span style="font-size:10px;color:#64748b;margin-left:4px;">Below threshold</span>' : ''}</td>
              <td class="num" style="color:#ef4444;">TZS ${paye.toLocaleString()}</td>
            </tr>
            <tr class="border-bottom">
              <td>NSSF (10%)</td>
              <td class="num" style="color:#ef4444;">TZS ${nssf.toLocaleString()}</td>
            </tr>
            ${other > 0 ? `<tr class="border-bottom"><td>Other</td><td class="num" style="color:#ef4444;">TZS ${other.toLocaleString()}</td></tr>` : ''}
            <tr class="total">
              <td>Total Deductions</td>
              <td class="num" style="color:#ef4444;">TZS ${row.ded.toLocaleString()}</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div class="net-hero">
      <div>
        ...
        <div class="net-sub">After all deductions &mdash; ${monthLabel}</div>
      </div>
      <div class="net-val">TZS ${net.toLocaleString()}</div>
    </div>

    <div class="tax-box">
      <strong>PAYE calculated per TRA Tanzania rates:</strong> 0% up to TZS 270,000 &mdash; 8% (270k&mdash;520k) &mdash; 20% (520k&mdash;760k) &mdash; 25% (760k&mdash;1M) &mdash; 30% above TZS 1,000,000
    </div>
    ...
    <div class="signatures">
      <div class="sig-col">
        ...
        <div class="sig-name">${row.emp}</div>
      </div>
      <div class="sig-col">
        <div class="sig-line"></div>
        <div class="sig-title">Authorised By</div>
        <div class="sig-name">HR / Finance Manager</div>
      </div>
    </div>

    <div class="ftr">
      This is a computer-generated payslip &mdash; Moovit ClearOS &mdash; Generated on ${new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })}
    </div>
  </div>
</body>
</html>`);
  ...
  return (
    <div onClick={onClose}
      ...
      <div onClick={e => e.stopPropagation()}
        ...
        {/* Actions bar */}
        <div style={{ padding: '12px 20px', borderBottom: '1px solid #e5e7eb', display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          <button type="button" className="btn btn-primary" onClick={() => printPayslip(row, monthLabel)}
