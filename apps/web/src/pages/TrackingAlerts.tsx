        ...
        {!loading && alerts.map(a => {
          ...
          return (
            <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 14, background: 'var(--white)', border: '1px solid var(--border)', borderRadius: 9, padding: '14px 18px', opacity: a.acknowledged ? 0.6 : 1 }}>
              ...
              {!a.acknowledged && (
                <button type="button" onClick={() => acknowledge(a.id)} style={{ fontSize: 12, fontWeight: 600, color: 'var(--teal)', background: 'none', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: 'var(--ds-btn-py-sm) 14px', cursor: 'pointer', flexShrink: 0, minHeight: 'var(--ctl-h-sm)', boxSizing: 'border-box'}}>
                  Acknowledge
                </button>
              )}
            </div>
