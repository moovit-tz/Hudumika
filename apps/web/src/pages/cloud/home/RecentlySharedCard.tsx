...
export function RecentlySharedCard({ items, onOpen }: { items: CloudFile[]; onOpen: (item: CloudFile) => void }) {
  ...
  // A "Recently Shared" section header still appears above this card even
  // with nothing shared yet (see CloudHome.tsx) — this used to vanish the
  // whole section instead, which meant the three-column row's third column
  // just disappeared rather than staying visually complete.
  if (shared.length === 0) {
    return (
      <Card style={{ padding: '28px 16px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, color: 'var(--ink3)' }}>
        <Icon name="userPlus" size={22} color="var(--border)" />
        <span style={{ fontSize: 12.5 }}>Nothing shared yet</span>
      </Card>
    );
  }
