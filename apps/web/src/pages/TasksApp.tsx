        ...
        {completed.length > 0 && view !== 'trash' && (
          <div style={{ marginTop: 24 }}>
            <button type="button" onClick={() => setShowCompleted(v => !v)}
              style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ink3)', fontSize: 13, fontWeight: 600, padding: 'var(--ds-btn-py-sm) 0', minHeight: 'var(--ctl-h-sm)', boxSizing: 'border-box'}}>
    ...
    <div className="list-row-accent" data-variant={!trashed && todo.status !== 'none' ? statusMeta.variant : undefined}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', opacity: todo.completed ? 0.6 : 1 }}>
        ...
        {!trashed && todo.status === 'none' && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button type="button" title="Set status" style={{ background: 'none', border: '1px dashed var(--border)', borderRadius: 20, cursor: 'pointer', padding: 'var(--ds-btn-py-xs) 9px', fontSize: 11, color: 'var(--ink3)', minHeight: 'var(--ctl-h-xs)', boxSizing: 'border-box'}}>+ Status</button>
            </DropdownMenuTrigger>
            <StatusMenuItems todoId={todo.id} />
          </DropdownMenu>
        )}
