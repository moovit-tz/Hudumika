...
function QuoteFormView({ mode, initial, customers, leads, onSave, onCancel, isMobile = false }: {
  ...
  async function submit(asDraft:boolean){
    if(!f.title.trim()){ alert('Please enter a quotation title.'); return; }
    if(!f.customer_name.trim()){ alert('Please select a customer or lead.'); return; }
    if(f.lines.length===0){ alert('Add at least one line item.'); return; }
    setSaving(true);
...
export const Quotations: React.FC = () => {
  ...
  async function handleConvert(id:string){
    if(!confirm('Convert to Shipment Case? This cannot be undone.')) return;
    const r = await apiFetch(`/v1/quotations/${id}/convert`,{method:'POST'});
    alert(`Shipment ${r.shipment?.ref_number??''} created!`);
    await fetchQuotes(); if(selected?.id===id) await fetchDetail(id);
  }
  ...
  async function handleDuplicate(id:string){
    try{ const r=await apiFetch(`/v1/quotations/${id}/duplicate`,{method:'POST'}); await fetchQuotes(); if(r?.id) await fetchDetail(r.id); else alert('Quotation duplicated.'); }
    catch{ alert('Duplicate not supported by API yet.'); }
  }
