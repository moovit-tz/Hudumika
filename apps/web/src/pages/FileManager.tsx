  ...
  return (
    <div style={{ width:300, flexShrink:0, borderLeft:'1px solid var(--border)', background:'var(--white)', display:'flex', flexDirection:'column', overflowY:'auto' }}>
      ...
      <div style={{ padding:'12px 16px', display:'flex', gap:8, borderBottom:'1px solid var(--border)' }}>
        ...
        <button onClick={()=>onShare(item)} style={{ flex: item.type==='folder' ? 1 : undefined, background:'var(--bg)', border:'1px solid var(--border)', borderRadius:'var(--r-sm)', padding:'var(--ds-btn-py-sm) 10px', cursor:'pointer', color:GD.blue }}>
          <Icon name="userPlus" size={14} color={GD.blue} />
        </button>
        <button onClick={()=>onStar(item)} style={{ background: item.starred ? 'var(--gold-l)' : 'var(--bg)', border:'1px solid var(--border)', borderRadius:'var(--r-sm)', padding:'var(--ds-btn-py-sm) 10px', cursor:'pointer', color: item.starred ? '#f59e0b' : 'var(--ink3)' }}>
          <Icon name="star" size={14} color={item.starred ? '#f59e0b' : 'var(--ink3)'} />
        </button>
        <button onClick={()=>onDelete(item)} style={{ background:'var(--bg)', border:'1px solid var(--border)', borderRadius:'var(--r-sm)', padding:'var(--ds-btn-py-sm) 10px', cursor:'pointer', color:'var(--red)' }}>
