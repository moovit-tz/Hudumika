          ...
          {breadcrumb.map((crumb, idx) => (
            <React.Fragment key={idx}>
              ...
              <button
                ...
                style={{ background: 'none', border: 'none', padding: 'var(--ds-btn-py-xs) 4px', borderRadius:'var(--r-sm)', cursor: idx < breadcrumb.length - 1 ? 'pointer' : 'default', color: idx === breadcrumb.length - 1 ? 'var(--ink)' : 'var(--ink3)', fontWeight: idx === breadcrumb.length - 1 ? 600 : 400, fontSize:'var(--text-md)' }}
              >
                {crumb.name}
              </button>
            </React.Fragment>
