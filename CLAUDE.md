...
**Dialogs / popups — a multi-step popup must not resize between steps.** `Dialog` / `DialogContent` (`ui/dialog.tsx`, Radix). Any popup with tabs, steps, or otherwise variable-height content per view **must** pass `size` (`sm`/`md`/`lg`/`xl`/`full`, footprints in `DIALOG_SIZES`) — or `steady` plus its own `w-*` / fixed `h-*` classes when it needs a bespoke width — and lay its content out as `DialogHeader` + `DialogBody` + `DialogFooter`. `size` pins the dialog to a fixed width *and* height (clamped to the viewport) that stays constant as the user moves between steps; `DialogHeader`/`DialogFooter` pin to the top/bottom and `DialogBody` is the *only* part that scrolls. A dialog that grows and shrinks as you click through its own tabs — the Contacts "Edit contact" modal did exactly this, jumping from ~380px to ~900px tall between Profile and Contact — is wrong and gets migrated. Never hand-roll a fixed height with `style={{ maxHeight, overflowY }}` on `DialogContent` itself; that scrolls the header and footer too. Omitting `size` keeps the legacy grow-to-fit behaviour and is fine for a genuinely single-view dialog (a confirm, a one-field prompt).
