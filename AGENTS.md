...
- **Stack, precisely:** React 19, React Router v6, Vite. **Tailwind is v4** (`@import "tailwindcss"` + `@theme{}` in `index.css`, `@tailwindcss/postcss`, no `tailwind.config.js`). CLAUDE.md said "v3" for a long time; that line is now corrected, but don't let a v3 assumption shape anything you write — reaching for a `tailwind.config.js`, or writing `w-[--some-var]` instead of `w-(--some-var)`, both fail silently rather than erroring. See CLAUDE.md's v4 syntax trap note for the second one, which had gone unnoticed in 28 places.
